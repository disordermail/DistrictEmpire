    ...
    public sealed class DistrictEmpireBootstrap : MonoBehaviour
    {
        ...
        private void Render()
        {
            ...
            switch (screen)
            {
                ...
                case "Exchange": RenderAuctions(); break;
            ...
            root.Add(BuildNav());
            if (screen == "Map" && mapSheetOpen) root.Add(BuildMapSheet());
        }
        ...
        private VisualElement BuildNav()
        {
            ...
            AddNavButton(nav, "Exchange", "EXCHANGE", "$");
        ...
        private void RenderToday()
        {
            ...
            rent.Add(copy);
            Button collect = null;
            collect = UiKit.Button(game.State.RentReady > 0 ? "$ Collect rent" : "View rent schedule", () =>
            {
                var amount = game.State.RentReady;
                if (amount <= 0) { ShowToast("The next payment is due tomorrow."); return; }
                AnimateActionCard(rent, collect, "COLLECTING...", () =>
                {
                    game.CollectRent();
                    Render();
                    ShowCelebration("RENT COLLECTED", "+" + Money(amount), "Maria paid her rent. +25 XP · +1 influence");
                });
            ...
            AddPropertyGroup("RENTING NOW", owned.Where(property => property.Stage == PropertyStage.Occupied && property.Condition >= 90).ToList());
        ...
        private VisualElement BuildMapSheet()
        {
            ...
            Button action = null;
            action = UiKit.Button(affordable ? MapFlowLabel(property) : "Need " + Money(property.Price - game.State.Cash), () =>
            {
                if (ShouldAnimateMapAction(property)) AnimateActionCard(sheet, action, MapActionWorkingLabel(property), () => RunMapAction(property));
                else RunMapAction(property);
            }, affordable ? MapActionKind(property) : "locked");
        ...
        private void ContinueManaging()
        {
            ...
            if (next == null) { screen = "Map"; mapSheetOpen = false; Render(); return; }
        ...
        private VisualElement NearbyPropertyCard(Property property)
        {
            var card = UiKit.Card("neutral"); card.AddToClassList("market-card");
            card.Add(UiKit.Text(property.Name, 16, true));
            card.Add(UiKit.Text(property.District + " · " + property.Category + " · " + Money(property.Price), 11, false, UiKit.Muted));
            card.Add(UiKit.Text("Potential " + Money(property.BaseDailyRent) + " / day", 12, true, UiKit.Green));
            card.Add(UiKit.Button("View on map", () => OpenPropertyFromMap(property), "secondary"));
            return card;
        }
        ...
        private bool ShouldAnimateMapAction(Property property) => !property.IsOwned || property.Condition < 90 || (property.Stage == PropertyStage.Occupied && game.State.RentReady > 0);
        private static string MapActionWorkingLabel(Property property) => !property.IsOwned ? "BUYING..." : property.Condition < 90 ? "REPAIRING..." : "COLLECTING...";
        private string MapActionKind(Property property) => !property.IsOwned ? "income" : property.Condition < 90 ? "repair" : property.Stage == PropertyStage.Occupied && game.State.RentReady > 0 ? "collect" : "primary";
        ...
        private static string PropertyActionHint(Property property)
        {
            ...
            if (property.Stage == PropertyStage.Listing) return "Hint: wait for applications. You can return to Portfolio while the listing runs.";
        ...
        private void AddExchangeGroup(string title, string subtitle, System.Collections.Generic.IEnumerable<Property> properties)
        {
            var listings = properties.OrderBy(property => property.Price).ToList();
            if (listings.Count == 0) return;
            content.Add(SectionHeading(title, subtitle));
            foreach (var property in listings)
            {
                ...
                Button buy = null;
                buy = UiKit.Button(affordable ? "Place bid · " + Money(property.Price) : "Need " + Money(property.Price - game.State.Cash) + " more", () => AnimateActionCard(card, buy, "BUYING...", () => BuyFromMarket(property)), affordable ? "income" : "locked");
        ...
        {
            if (game.Buy(property.Id)) { selectedPropertyId = property.Id; screen = "Portfolio"; mapSheetOpen = false; ShowToast("Purchase accepted. Notary transfer has started."); }
        ...
        private void RenderTasks()
        {
            ...
            foreach (var property in tasks)
            {
                ...
                card.Add(UiKit.Button("Repair for 450 PLN", () => { if (game.Repair(property.Id)) ShowToast("Maintenance completed. Condition improved."); else ShowToast("You need 450 PLN for this repair."); Render(); }, "repair")); content.Add(card);
            }
        }
        ...
        private void RenderProperty()
        {
            ...
            {
                Button buy = null;
                buy = UiKit.Button("Buy for " + Money(property.Price), () => AnimateActionCard(hero, buy, "BUYING...", () => BuyFromMarket(property)), "income");
                content.Add(buy);
            }
            ...
            else if (property.Stage == PropertyStage.ChoosingUse)
            {
                ...
                choice.Add(UiKit.Button("Rent as a home", () => { game.ChooseUse(property.Id, PropertyUse.Residential); Render(); }, "income"));
                choice.Add(UiKit.Button("Use as a business", () => { game.ChooseUse(property.Id, PropertyUse.Business); Render(); }, "income")); content.Add(choice);
            }
            else if (property.Stage == PropertyStage.Available)
            {
                ...
                Button publish = null;
                publish = UiKit.Button("Publish rental listing", () => AnimateActionCard(advertise, publish, "PUBLISHING...", () => { game.PublishListing(property.Id); Render(); }), "income");
                advertise.Add(publish); content.Add(advertise);
            }
            ...
            else if (property.Stage == PropertyStage.ForSale)
            {
                ...
                sale.Add(UiKit.Button("Accept sale offer", () => { if (game.AcceptSaleOffer(property.Id)) { screen = "Portfolio"; Render(); ShowCelebration("PROPERTY SOLD", "+" + Money(saleValue), "Sale completed. Your cash and company XP increased."); } }, "danger")); content.Add(sale);
            }
            else if (property.Stage == PropertyStage.Applications)
            {
                ...
                foreach (var applicant in property.Applicants)
                {
                    ...
                    Button accept = null;
                    accept = UiKit.Button("Accept applicant", () => AnimateActionCard(applicantCard, accept, "SIGNING...", () => { game.SelectApplicant(property.Id, applicant.Id); Render(); ShowCelebration("LEASE SIGNED", applicant.Name, "The property is now earning rent."); }), "income");
                    applicantCard.Add(accept); content.Add(applicantCard);
                }
            }
            else
            {
                ...
                if (property.Condition < 90)
                {
                    Button maintain = null;
                    maintain = UiKit.Button("Maintain property", () => AnimateActionCard(occupied, maintain, "REPAIRING...", () => { game.Repair(property.Id); Render(); ShowCelebration("REPAIR COMPLETE", "Condition improved", "Your tenant can get back to normal."); }), "repair");
                    occupied.Add(maintain);
                }
                content.Add(occupied);
        ...
        private void AnimateActionCard(VisualElement card, Button button, string workingLabel, Action complete)
        {
            if (card == null || button == null) { complete(); return; }
            button.SetEnabled(false);
            button.text = workingLabel;
            card.AddToClassList("action-card-working");
            AddActionSparks(card);
            card.schedule.Execute(() =>
            {
                if (card.parent == null) return;
                card.RemoveFromClassList("action-card-working");
                card.AddToClassList("action-card-done");
                button.text = "DONE";
                button.style.backgroundColor = UiKit.Green;
            }).StartingIn(420);
            for (var step = 1; step <= 5; step++)
            {
                ...
                {
                    if (card.parent == null) return;
                    var progress = frame / 5f;
                    card.style.scale = new Scale(new Vector2(1f - progress * 0.04f, 1f - progress * 0.04f));
                    card.style.translate = new Translate(new Length(360f * progress, LengthUnit.Pixel), new Length(0, LengthUnit.Pixel));
                    card.style.opacity = 1f - progress;
                }).StartingIn(760 + frame * 70);
            }
            card.schedule.Execute(complete).StartingIn(1160);
        }

        private static void AddActionSparks(VisualElement card)
        {
            var sparks = new VisualElement(); sparks.AddToClassList("action-sparks");
            for (var index = 0; index < 5; index++)
            {
                var spark = new VisualElement(); spark.AddToClassList("action-spark");
                spark.style.left = 24 + index * 42;
                spark.style.top = index % 2 == 0 ? 14 : 34;
                sparks.Add(spark);
            }
            card.Add(sparks);
            sparks.schedule.Execute(() => { if (sparks.parent != null) sparks.parent.Remove(sparks); }).StartingIn(900);
        }
